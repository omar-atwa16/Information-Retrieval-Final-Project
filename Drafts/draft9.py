...
def invertedIDX(docs: Dict[str, str]) -> Dict[str, Set[str]]:
    ...
    for docID, text in docs.items():
        # Skip if already indexed (check any existing token)
        if any(docID in invertedIDX.get(token, set()) for token in invertedIDX):
            continue
...
# STREAMLIT APP
# =========================
st.set_page_config(page_title="PubMed Mini Search Engine", page_icon="🔍", layout="wide")
st.title("🔍 PubMed Mini Search Engine")
...
with tab1:
    ...
    pmid_input = st.text_input("Enter PubMed ID (PMID):", placeholder="e.g., 32361569")
...
with tab2:
    ...
    col1, col2 = st.columns(2)
    with col1:
        # Year range slider instead of dropdown
        current_year = datetime.now().year
        year_range = st.slider(
            "Filter by Year Range:",
            min_value=1980,
            max_value=current_year,
            value=(1980, current_year),  # Default: last few years
            step=1,
            format="%d"
        )
        st.caption(f"Selected: {year_range[0]} to {year_range[1]}")
    with col2:
    ...
    if st.button("Search Articles", key="search_multiple"):
        if query_input:
            with st.spinner("Searching PubMed..."):
                # Modify query to include year range
                year_query = f"{query_input} AND {year_range[0]}:{year_range[1]}[pdat]"
                pmids = searchPubMed(year_query, max_results, year=None)  # Pass None since year is in query
                
                if pmids:
                    # Store search in history
                    search_entry = {
                        "query": query_input,
                        "year": f"{year_range[0]}-{year_range[1]}",  # Store as range
                        "timestamp": datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                        "retrieved_docs": pmids,
                        "relevant_docs": set(pmids)
                    }
                    st.session_state["search_history"].append(search_entry)
                    
                    st.success(f"Found {len(pmids)} articles from {year_range[0]} to {year_range[1]}. Fetching details...")
            ...
            if not search_token:
                ...
                if not processed_tokens:
                    ...
                    if len(processed_tokens) == 1:
                        matching_pmids = inverted_index.get(processed_tokens[0], set())
                        search_token_processed = processed_tokens[0]  # ✓ Define it here
                    else:
                        # AND logic: documents containing ALL tokens
                        matching_pmids = set.intersection(*[
                            inverted_index.get(token, set()) for token in processed_tokens
                        ]) if processed_tokens else set()
                        search_token_processed = ' AND '.join(processed_tokens)  # ✓ Define it here
